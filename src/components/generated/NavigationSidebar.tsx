...
const NavItem = ({
  ...
  return <button onClick={onClick} className={cn("group relative flex flex-col items-center justify-center w-[58px] h-[58px] rounded-md transition-all duration-200 cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-[#520044]/30", isActive ? "bg-white shadow-sm" : "hover:bg-black/5")} aria-label={label}>
...
export const NavigationSidebar = () => {
  ...
  return <aside className="fixed left-0 top-0 h-full flex flex-col items-center bg-[#f4f4f4] border-r border-gray-200 z-50">
      ...
      <nav className="flex-1 w-[66px] flex flex-col items-center py-3 gap-1.5" aria-label="Fandom navigation">
        {menuItems.map(item => <NavItem key={item.id} icon={item.icon} label={item.label} badge={item.badge} isActive={activeTab === item.id} onClick={() => setActiveTab(item.id)} />)}
